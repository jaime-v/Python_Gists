...
# import other files
from backend.app.schemas import ItemIn, ItemOut, UserIn, UserOut, TestUser, TestUserInDB, TestToken, TestTokenData
...
@app.post("/token/")
async def login(form_data: Annotated[OAuth2PasswordRequestForm, Depends()]) -> TestToken:
...
@app.get("/users/me/")
async def get_users_me(current_user: Annotated[TestUser, Depends(get_current_active_user)]) -> TestUser:
    return current_user

@app.get("/users/me/items/")
async def get_users_me_items(current_user: Annotated[TestUser, Depends(get_current_active_user)]):
